...
public class WandOfFireblast extends DamageWand {
				...
				for (int i : PathFinder.NEIGHBOURS9) {
					...
					if (ch != null) {
						...
						if (ch.alignment == Char.Alignment.ENEMY) {
							...
							ch.damage(Math.round(powerMulti*Random.NormalIntRange(2 + 2*buffedLvl(), 8 + 4*buffedLvl())), new Char.DamageCause(this, attacker));
						}
					}
				}
			}
	...
	protected int chargesPerCast() {
		...
		return (int) GameMath.gate(1, (int)Math.ceil(getCurCharges() *0.3f), 3);
	}
