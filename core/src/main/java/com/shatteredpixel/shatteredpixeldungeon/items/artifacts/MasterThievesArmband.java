...
public class MasterThievesArmband extends Artifact {
	...
	public class Thievery extends ArtifactBuff {
		...
		@Override
		public boolean act() {
			if (target instanceof Hero && cursed && ((Hero) target).getGold() > 0 && Random.Int(5) == 0){
				((Hero) target).setGold(((Hero) target).getGold()-1);
				updateQuickslot();
			}
		...
		public boolean steal(Item item){
			...
			if (Random.Float() > stealChance){
				...
				setCharge(getCharge() - chargesUsed);
				exp += 4 * chargesUsed;
		...
		public int chargesToUse(Item item){
			...
			float valUsing = 0;
			int chargesUsed = 0;
			while (valUsing < value && chargesUsed < getCharge()){
