...
abstract public class MissileWeapon extends Weapon {
	...
	public int min(Char owner) {
		if (owner != null){
			return Math.max(0, min(buffedLvl() + RingOfSharpshooting.levelDamageBonus(owner)));
		} else {
	...
	public Item upgrade( boolean enchant, Hero hero ) {
		if (!bundleRestoring) {
			durability = MAX_DURABILITY;
			extraThrownLeft = false;
			quantity = defaultQuantity();
			Buff.affect(hero, UpgradedSetTracker.class).levelThresholds.put(setID, trueLevel()+1);
		}
	...
	public boolean isSimilar( Item item ) {
		return trueLevel() == item.trueLevel() && getClass() == item.getClass() && setID == (((MissileWeapon) item).setID) && item.fragmentUpgrades.equals(fragmentUpgrades);
	}
	...
	public int proc(Char attacker, Char defender, int damage) {
		...
		if (attacker instanceof Hero && ((Hero) attacker).pointsInTalent(Talent.SURVIVALISTS_INTUITION) == 2){
	...
	public final float durabilityPerUse(@Nullable Hero hero){
		return durabilityPerUse(level(), hero);
	}
	...
	public float durabilityPerUse( int level, @Nullable Hero hero ){
	...
	protected void decrementDurability(@Nullable Hero hero){
