...
public class ChaoticCenser extends Trinket {
	...
	private static boolean produceGas(){
		...
		PathFinder.buildDistanceMap(target.pos, BArray.not(Dungeon.level.solid, null), 5);
		...
		if (!candidateCells.isEmpty()) {
			...
			if (targetCell != null) {
				...
				MagicMissile.boltFromChar(target.getSprite().parent, MISSILE_VFX.get(gasToSpawn), target.getSprite(), targetCell, null);
				return true;
			}
		}
