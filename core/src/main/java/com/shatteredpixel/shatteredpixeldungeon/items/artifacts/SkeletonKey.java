...
public class SkeletonKey extends Artifact {
			...
			if (target != null && (Dungeon.level.visited[target] || Dungeon.level.mapped[target])){
				...
				if (Dungeon.level.adjacent(target, curUser.pos)) {
					...
					if (Dungeon.level.map[target] == Terrain.LOCKED_DOOR){
						...
						curUser.getSprite().operate(target, new Callback() {
							@Override
							public void call() {
								Buff.affect(curUser, KeyReplacementTracker.class).processIronLockOpened();
								Level.set(target, Terrain.DOOR);
								GameScene.updateMap(target);
								setCharge(getCharge() - 1, getOwner());
								gainExp(2 + 1);
								Talent.onArtifactUsed(getOwner());
						...
						curUser.getSprite().operate(target, new Callback() {
							...
							public void call() {
								...
								GameScene.updateMap(target);
								setCharge(getCharge() - 5, getOwner());
								gainExp(2 + 5);
								Talent.onArtifactUsed(getOwner());
						...
						if (Actor.findChar(target) != null){
							...
							if (pushCell != -1 && !Char.hasProp(toMove, Char.Property.IMMOVABLE)){
								...
								artifactProc(toMove, visiblyUpgraded(), 2, getOwner());
							} else {
						...
						curUser.getSprite().operate(target, new Callback() {
							...
							public void call() {
								...
								GameScene.updateMap(target);
								setCharge(getCharge() - 2, getOwner());
								gainExp(2);
								Talent.onArtifactUsed(getOwner());
								curUser.spendAndNext(Actor.TICK);
								curUser.getSprite().idle();
				...
				curUser.getSprite().operate(target, new Callback() {
					@Override
					public void call() {
						placeWall(curUser.pos+PathFinder.CIRCLE8[finalClosestIdx], knockBackDir, getOwner());
						placeWall(curUser.pos+PathFinder.CIRCLE8[(finalClosestIdx +7)%8], knockBackDir, getOwner());
						placeWall(curUser.pos+PathFinder.CIRCLE8[(finalClosestIdx +1)%8], knockBackDir, getOwner());

						//if we're in a diagonal direction
						if (finalClosestIdx % 2 == 0){
							placeWall(curUser.pos+2*PathFinder.CIRCLE8[(finalClosestIdx +7)%8], knockBackDir, getOwner());
							placeWall(curUser.pos+2*PathFinder.CIRCLE8[(finalClosestIdx +1)%8], knockBackDir, getOwner());
						}
						setCharge(getCharge() - 2);
						gainExp(2);
	...
	public class keyRecharge extends ArtifactBuff {
		...
		public boolean act() {
			if (getCharge() < chargeCap
				...
				float chargeGain = 1 / (120f - (chargeCap - getCharge())*7.5f);
	...
	private void placeWall(int pos, int knockbackDIR, Hero hero ){
		...
		if (!Dungeon.level.solid[pos] || (wall != null && wall.cur[pos] > 0)) {
			...
			if (ch != null && ch.alignment == Char.Alignment.ENEMY){
				...
				artifactProc(ch, visiblyUpgraded(), 2, hero);
			}
		}
	}
