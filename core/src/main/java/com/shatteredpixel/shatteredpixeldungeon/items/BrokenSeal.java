				...
				GameScene.show(new WndOptions(new ItemSprite(ItemSpriteSheet.SEAL),
					...
					protected void onSelect(int index) {
						...
						if (outgoing == BrokenSeal.this) {
							...
							((Armor) outgoing).detachSeal(hero);
						}
						...
						armor.affixSeal(BrokenSeal.this, hero);
					}
	...
	protected static WndBag.ItemSelector armorSelector = new WndBag.ItemSelector() {
		...
		public void onSelect( Item item ) {
			if (item instanceof Armor) {
				...
				seal.affixToArmor((Armor)item, seal, getOwner());
			}
		}
	};
	...
	public static class WarriorShield extends ShieldBuff {
		...
		public synchronized boolean act() {
            ...
            if (shielding() > 0) {
                ...
                if (hero.visibleEnemies() == 0 && hero.buff(Combo.class) == null) {
            ...
            if (shielding() <= 0 && maxShield((Hero) target) <= 0 && cooldown == 0) {
                detach();
                return true;
            }
		...
		public synchronized void activate(Hero hero) {
			incShield(maxShield(hero));
			cooldown = Math.max(0, cooldown+COOLDOWN_START);
			turnsSinceEnemies = 0;
			initialShield = maxShield(hero);
		}
