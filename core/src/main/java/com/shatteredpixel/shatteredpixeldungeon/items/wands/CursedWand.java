...
public class CursedWand {
	...
	public static void cursedZap(final Item origin, final Char user, final Ballistica bolt, final Callback afterZap){
		...
		if (user instanceof Hero) {
			Hero owner = (Hero) user;
			 positiveOnly = user instanceof Hero && Random.Float() < WondrousResin.positiveCurseEffectChance(owner);
		}
		CursedEffect effect = randomValidEffect(origin, user, bolt, positiveOnly);

		final boolean finalPositiveOnly = positiveOnly;
		effect.FX(origin, user, bolt, new Callback() {
			@Override
			public void call() {
				effect.effect(origin, user, bolt, finalPositiveOnly);
				afterZap.call();
			}
		});
	}
	...
	public static class SpawnRegrowth extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean postiveOnly) {
			...
			if (user instanceof Hero) {
					tryForWandProc(Actor.findChar(bolt.collisionPos), origin, (Hero) user);
				}
			return true;
		}
	}
	...
	public static class HealthTransfer extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			if (target != null) {
				...
				toHeal.setHP(Math.min(toHeal.getHT(), toHeal.getHP() + damage/2));
	...
	public static class LightningBolt extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			user.getSprite().parent.add(new Lightning(user.pos - 1, user.pos + 1, null));
			user.getSprite().parent.add(new Lightning(user.pos - Dungeon.level.width(), user.pos + Dungeon.level.width(), null));
			user.getSprite().parent.add(new Lightning(user.pos - 1 - Dungeon.level.width(), user.pos + 1 + Dungeon.level.width(), null));
			user.getSprite().parent.add(new Lightning(user.pos - 1 + Dungeon.level.width(), user.pos + 1 - Dungeon.level.width(), null));
			for (int i : PathFinder.NEIGHBOURS9){
			...
			int pos = bolt.collisionPos;
			user.getSprite().parent.add(new Lightning(pos - 1, user.pos + 1, null));
			user.getSprite().parent.add(new Lightning(pos - Dungeon.level.width(), pos + Dungeon.level.width(), null));
			user.getSprite().parent.add(new Lightning(pos - 1 - Dungeon.level.width(), pos + 1 + Dungeon.level.width(), null));
			user.getSprite().parent.add(new Lightning(pos - 1 + Dungeon.level.width(), pos + 1 - Dungeon.level.width(), null));
			for (int i : PathFinder.NEIGHBOURS9){
			...
			for (Char ch : affected){
				...
				if (ch.alignment != Char.Alignment.ALLY || !positiveOnly){
					...
					ch.damage(Random.NormalIntRange(5 + Dungeon.scalingDepth() / 4, 10 + Dungeon.scalingDepth() / 4), new Char.DamageCause( new Electricity(), null));
					if (ch.isAlive()) {
	...
	public static class Alarm extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			user.getSprite().centerEmitter().start( Speck.factory( Speck.SCREAM ), 0.3f, 3 );
			if (positiveOnly){
	...
	public static class InterFloorTeleport extends CursedEffect {
		...
		@Override
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			if (!positiveOnly && Dungeon.depth > 1 && Dungeon.interfloorTeleportAllowed() && user instanceof Hero) {
	...
	public static class SummonMonsters extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			if (positiveOnly && user instanceof Hero){
				ScrollOfMirrorImage.spawnImages((Hero) user, bolt.collisionPos, 2);
			} else {
	...
	public static class FireBall extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			WandOfBlastWave.BlastWave.blast(bolt.collisionPos, 6, user);
	...
	public static class ConeOfColors extends CursedEffect {
		...
		public void FX(Item origin, Char user, Ballistica bolt, Callback callback) {
			...
			for (Ballistica ray : cone.outerRays){
				...
				((MagicMissile) user.getSprite().parent.recycle( MagicMissile.class )).reset(
	...
	public static class MassInvuln extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			new Flare(5, 48).color(0xFFFF00, true).show(user.getSprite(), 3f);
	...
	public static class Petrify extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			user.getSprite().emitter().burst(Speck.factory(Speck.STEAM), 10);
	...
	public static class RandomTransmogrify extends CursedEffect {
		...
		public boolean valid(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			if (positiveOnly) {
				return true;
			} else if (origin == null || !(user instanceof Hero) /*FIXME || !Dungeon.hero.belongings.contains(origin)*/) {
				return false;
			} else {
				return true;
			}
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			origin.detach(((Hero)user).belongings.backpack);
				...
				Dungeon.level.drop(result, user.pos).sprite.drop();return true;
		}
	}

	public static class HeroShapeShift extends CursedWand.CursedEffect {
	...
	public static class SuperNova extends CursedEffect {
		@Override
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			SuperNovaTracker nova = Buff.append(user, SuperNovaTracker.class);
	...
	public static class SinkHole extends CursedWand.CursedEffect {
	...
	static class GravityChaos extends CursedWand.CursedEffect {
