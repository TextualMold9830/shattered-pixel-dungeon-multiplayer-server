...
public class WndUpgrade extends Window {
	...
	public WndUpgrade(Item upgrader, Item toUpgrade, boolean force, Hero owner){
		...
		if (toUpgrade instanceof Weapon){
			...
			bottom = fillFields(Messages.get(this, "damage"),
					...
					aug.damageFactor(((Weapon) toUpgrade).min(levelTo, owner)) + "-" + aug.damageFactor(((Weapon) toUpgrade).max(levelTo, getOwnerHero())),
					bottom);
		}
		...
		if (toUpgrade instanceof MissileWeapon){
			...
			int uses1 = (int)Math.ceil(100f/((MissileWeapon) toUpgrade).durabilityPerUse(owner));
			int uses2 = (int)Math.ceil(300f/((MissileWeapon) toUpgrade).durabilityPerUse(owner));
			...
			if (((Ring) toUpgrade).isKnown()) {
				if (((Ring) toUpgrade).upgradeStat1(levelFrom) != null) {
					bottom = fillFields(Messages.get(toUpgrade, "upgrade_stat_name_1"),
							((Ring) toUpgrade).upgradeStat1(levelFrom, getOwnerHero()),
							((Ring) toUpgrade).upgradeStat1(levelTo, getOwnerHero()),
							bottom);
				}
				if (((Ring) toUpgrade).upgradeStat2(levelFrom) != null) {
					bottom = fillFields(Messages.get(toUpgrade, "upgrade_stat_name_2"),
							((Ring) toUpgrade).upgradeStat2(levelFrom, getOwnerHero()),
							((Ring) toUpgrade).upgradeStat2(levelTo, getOwnerHero()),
							bottom);
				}
				if (((Ring) toUpgrade).upgradeStat3(levelFrom) != null) {
					bottom = fillFields(Messages.get(toUpgrade, "upgrade_stat_name_3"),
							((Ring) toUpgrade).upgradeStat3(levelFrom, getOwnerHero()),
							((Ring) toUpgrade).upgradeStat3(levelTo, getOwnerHero()),
							bottom);
				}
			}
		}
		...
		btnUpgrade = new RedButton(Messages.get(this, "upgrade")){
			...
			protected void onClick() {
				...
				Item moreUpgradeItem = getOwnerHero().belongings.getItem(upgrader.getClass());
		...
		btnUpgrade.enable(getOwnerHero().ready);
	...
	public synchronized void update() {
		super.update();
		if (!btnUpgrade.active && getOwnerHero().ready){
