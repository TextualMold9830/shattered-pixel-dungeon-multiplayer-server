		...
		if (phase == 0){
			...
			if (!enemy.rooted) {
				...
				for (Char ch : affected) {
					...
					if (hit( this, ch, true )) {
						if (Dungeon.isChallenged(Challenges.STRONGER_BOSSES)) {
							ch.damage(Random.NormalIntRange(30, 50), new DamageCause(new Eye.DeathGaze(), this));
						} else {
							ch.damage(Random.NormalIntRange(20, 30), new DamageCause(new Eye.DeathGaze(), this));
						}
						if (Dungeon.visibleforAnyHero(pos)) {
							ch.getSprite().flash();
