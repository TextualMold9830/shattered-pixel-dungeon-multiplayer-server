...
public class Berserk extends ShieldBuff implements ActionIndicator.Action {
	...
	public boolean act() {
		if (state == State.BERSERK){
			...
			if (powerLossBuffer > 0){
				...
				power -= GameMath.gate(0.1f, power, 1f) * 0.05f * Math.pow((target.getHP() / (float) target.getHT()), 2);
	...
	public int currentShieldBoost(){
		...
		float shieldMultiplier = 1f + 2*(float)Math.pow((1f-(target.getHP() /(float) target.getHT())), 3);
	...
	public void damage(int damage){
		...
		power = Math.min(maxPower, power + (damage/(float) target.getHT())/4f );
