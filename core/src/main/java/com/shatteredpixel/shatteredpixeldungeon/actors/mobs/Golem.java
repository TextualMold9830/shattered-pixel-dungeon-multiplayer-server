...
public class Golem extends Mob {
	...
	public float lootChance(Hero hero) {
		...
		return super.lootChance(hero) * (float)Math.pow(1/2f, Dungeon.LimitedDrops.GOLEM_EQUIP.count);
	}
	...
	protected boolean act() {
		...
		enemyTeleCooldown--;
		if (teleporting){
			((GolemSprite) getSprite()).teleParticles(false);
	...
	private class Wandering extends Mob.Wandering{
		...
		protected boolean continueWandering() {
			...
			if (target != -1 && getCloser( target )) {
				...
				((GolemSprite) getSprite()).teleParticles(true);
				teleporting = true;
				spend( 2*TICK );
			} else {
