...
public class VaultMob extends Mob {
	...
	public void move(int step, boolean travelling) {
		...
		for (Hero hero: Dungeon.heroes) {
			if (hero != null) {
				if (travelling && !getSprite().visible && Dungeon.level.distance(pos, hero.pos) <= 6) {
					if (state == HUNTING) {
						WandOfBlastWave.BlastWave.blast(pos, 1f, 0xFF0000, this);
					} else if (state == INVESTIGATING) {
						WandOfBlastWave.BlastWave.blast(pos, 1f, 0xFF8800, this);
					} else {
						WandOfBlastWave.BlastWave.blast(pos, 1f, this);
					}
				}
			}
		}
	...
	public class Wandering extends Mob.Wandering {
		...
		protected boolean noticeEnemy() {
			...
			state = INVESTIGATING;
			investigatingTurns = 0;
			getSprite().showInvestigate();
			spend(TICK);
			//hero must know if they are detected
			for (Hero hero : Dungeon.heroes) {
				if (hero != null) {
					if (!hero.fieldOfView[pos]) {
						Buff.affect(hero, TalismanOfForesight.CharAwareness.class, 1f).charID = id();
					}
				}
			}
