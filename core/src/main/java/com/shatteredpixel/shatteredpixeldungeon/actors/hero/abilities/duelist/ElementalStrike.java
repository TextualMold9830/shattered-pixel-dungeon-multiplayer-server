	...
	private void preAttackEffect(ConeAOE cone, Hero hero, Weapon.Enchantment ench){
		...
		//*** Vampiric ***
			...
			if (targetsHit > 0){
				...
				heal = Math.min( heal, hero.getHT() - hero.getHP());
				if (heal > 0){
	...
	private void perCharEffect(ConeAOE cone, Hero hero, Char primaryTarget, Weapon.Enchantment ench) {
		...
		if (ench == null) {
			for (Char ch : affected){
				ch.damage(Math.round(powerMulti* Hero.heroDamageIntRange(6, 12)), new Char.DamageCause( ElementalStrike.this, hero));
			}
		...
		//*** Kinetic ***
			...
			if (storedKineticDamage > 0) {
				for (Char ch : affected) {
					if (ch != primaryTarget) {
						ch.damage(Math.round(storedKineticDamage * 0.4f * powerMulti), new Char.DamageCause( ench, hero));
					}
				}
		...
		//*** Projecting ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					ch.damage(Math.round(hero.damageRoll() * 0.3f * powerMulti), new Char.DamageCause( ench, hero));
				}
			}
		...
		//*** Corrupting ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget
						...
						&& ch instanceof Mob
						&& ch.isAlive()) {
					float hpMissing = 1f - (ch.getHP() / (float) ch.getHT());
		...
		//*** Grim ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					...
					if (Random.Float() < chance*powerMulti){
						ch.damage(ch.getHP(), new Char.DamageCause( Grim.class, hero) );
		...
		//*** Polarized ***
			...
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					ch.damage(Hero.heroDamageIntRange(24, 36), new Char.DamageCause( ElementalStrike.this, hero));
				}
			}
