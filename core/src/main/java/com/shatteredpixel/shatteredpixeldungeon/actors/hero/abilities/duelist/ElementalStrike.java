	...
	private void perCharEffect(ConeAOE cone, Hero hero, Char primaryTarget, Weapon.Enchantment ench) {
		...
		if (ench == null) {
			for (Char ch : affected){
				ch.damage(Math.round(powerMulti* Char.combatRoll(6, 12)), new Char.DamageCause( ElementalStrike.this, hero));
			}
		...
		//*** Polarized ***
			...
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					ch.damage(Char.combatRoll(24, 36), new Char.DamageCause( ElementalStrike.this, hero));
				}
			}
