...
public enum Talent {
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		if (hero.hasTalent(HEARTY_MEAL)){
			...
			if (hero.getHP() /(float) hero.getHT() < 0.334f) {
		...
		if (hero.hasTalent(ENLIGHTENING_MEAL)){
			if (hero.heroClass == HeroClass.CLERIC) {
				...
				if (tome != null) {
					...
					tome.directCharge( (1+hero.pointsInTalent(ENLIGHTENING_MEAL))/3f , hero);
					ScrollOfRecharging.charge(hero);
				}
			} else {
	...
	public static void onPotionUsed( Hero hero, int cell, float factor ){
		if (hero.hasTalent(LIQUID_WILLPOWER)){
			// 6.5/10% of max HP
			int shieldToGive = Math.round( factor * hero.getHT() * (0.030f + 0.035f*hero.pointsInTalent(LIQUID_WILLPOWER)));
