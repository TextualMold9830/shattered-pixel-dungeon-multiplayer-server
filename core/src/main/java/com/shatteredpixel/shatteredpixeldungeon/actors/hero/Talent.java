...
public enum Talent {
	...
	public static void onTalentUpgraded( Hero hero, Talent talent ){
		...
		if (talent == VETERANS_INTUITION && hero.pointsInTalent(VETERANS_INTUITION) == 2){
			if (hero.belongings.armor() != null && !ShardOfOblivion.passiveIDDisabled())  {
				hero.belongings.getRealArmor().identify(hero);
			}
		}
		if (talent == THIEFS_INTUITION && hero.pointsInTalent(THIEFS_INTUITION) == 2){
			if (hero.belongings.getRealRing() instanceof Ring && !ShardOfOblivion.passiveIDDisabled()) {
				hero.belongings.getRealRing().identify(hero);
			}
			if (hero.belongings.getRealMisc() instanceof Ring && !ShardOfOblivion.passiveIDDisabled()) {
				hero.belongings.getRealMisc().identify(hero);
			}
		...
		if (talent == ADVENTURERS_INTUITION && hero.pointsInTalent(ADVENTURERS_INTUITION) == 2){
			if (hero.belongings.weapon() != null && !ShardOfOblivion.passiveIDDisabled()){
				hero.belongings.weapon().identify(hero);
			}
		}
		...
		if (talent == UNENCUMBERED_SPIRIT && hero.pointsInTalent(talent) == 3){
			Item toGive = new ClothArmor().identify(hero);
			if (!toGive.collect(hero)){
				Dungeon.level.drop(toGive, hero.pos).sprite.drop();
			}
			toGive = new Gloves().identify(hero);
			if (!toGive.collect(hero)){
				Dungeon.level.drop(toGive, hero.pos).sprite.drop();
			}
		}
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		if (hero.hasTalent(HEARTY_MEAL)){
			...
			if (hero.getHP() /(float) hero.getHT() <= 0.3f) {
				int healing = 1 + 2 * hero.pointsInTalent(HEARTY_MEAL);
				hero.setHP(Math.min(hero.getHP() + healing, hero.getHT()));
				hero.getSprite().showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(healing), FloatingText.HEALING);
		...
		if (hero.hasTalent(FOCUSED_MEAL)){
			if (hero.heroClass == HeroClass.DUELIST){
				...
				Buff.affect( hero, MeleeWeapon.Charger.class ).gainCharge((hero.pointsInTalent(FOCUSED_MEAL)+1)/3f, hero);
				ScrollOfRecharging.charge( hero );
			} else {
		...
		if (hero.hasTalent(LIQUID_WILLPOWER)){
			if (hero.heroClass == HeroClass.WARRIOR) {
				...
				if (shield != null) {
					...
					int shieldToGive = Math.round(factor * shield.maxShield(hero) * 0.25f * (1 + hero.pointsInTalent(LIQUID_WILLPOWER)));
					hero.getSprite().showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(shieldToGive), FloatingText.SHIELDING);
					shield.supercharge(shieldToGive);
				}
			} else {
				// 5/7.5% of max HP
				int shieldToGive = Math.round( factor * hero.getHT() * (0.025f * (1+hero.pointsInTalent(LIQUID_WILLPOWER))));
				hero.getSprite().showStatusWithIcon(CharSprite.POSITIVE, Integer.toString(shieldToGive), FloatingText.SHIELDING);
