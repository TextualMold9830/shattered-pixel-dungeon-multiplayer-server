	...
	private void affectChar( Char ch ){
		if (ch.alignment == Char.Alignment.ALLY){
			if (ch instanceof Hero || ch.getHP() == ch.getHT()){
				...
				ch.getSprite().showStatusWithIcon( CharSprite.POSITIVE, Integer.toString(barrierToGive), FloatingText.SHIELDING );
			} else {
				int barrier = 15 - (ch.getHT() - ch.getHP());
				barrier = Math.max(barrier, 0);
				ch.setHP(ch.getHP() + 15 - barrier);
				ch.getSprite().showStatusWithIcon( CharSprite.POSITIVE, Integer.toString(15-barrier), FloatingText.HEALING );
				if (barrier > 0){
						...
						if (c == Terrain.GRASS && Dungeon.level.plants.get(c) == null) {
							if (Random.Int(chance) == 0) {
								if (!Regeneration.regenOn()
										|| (source.buff(HallowedFurrowTracker.class) != null && source.buff(HallowedFurrowTracker.class).count() > 100)){
