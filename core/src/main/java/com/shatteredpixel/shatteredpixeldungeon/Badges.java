...
public class Badges {
	...
	public static final String BADGES_FILE	= "save/badges.dat";
	...
	public static void validateStrengthAttained(Hero hero) {
		...
		if (!local.contains( Badge.STRENGTH_ATTAINED_1 ) && hero.getSTR() >= 12) {
			badge = Badge.STRENGTH_ATTAINED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.STRENGTH_ATTAINED_2 ) && hero.getSTR() >= 14) {
			if (badge != null) unlock(badge);
			badge = Badge.STRENGTH_ATTAINED_2;
			local.add( badge );
		}
		if (!local.contains( Badge.STRENGTH_ATTAINED_3 ) && hero.getSTR() >= 16) {
			if (badge != null) unlock(badge);
			badge = Badge.STRENGTH_ATTAINED_3;
			local.add( badge );
		}
		if (!local.contains( Badge.STRENGTH_ATTAINED_4 ) && hero.getSTR() >= 18) {
			if (badge != null) unlock(badge);
			badge = Badge.STRENGTH_ATTAINED_4;
			local.add( badge );
		}
		if (!local.contains( Badge.STRENGTH_ATTAINED_5 ) && hero.getSTR() >= 20) {
			if (badge != null) unlock(badge);
...
//		}
	...
	public static void validateTakingTheMick(Object cause){
		// Explosive.ExplosiveCurseBomb should also be validated
		if ((cause instanceof Hero)
				&& ((Hero) cause).belongings.attackingWeapon() instanceof Pickaxe
				&& ((Hero) cause).belongings.attackingWeapon().level() >= 20){
	...
	public static void validateNoKilling() {
		if (!local.contains( Badge.NO_MONSTERS_SLAIN ) && Statistics.completedWithNoKilling && Dungeon.depth > 1) {
