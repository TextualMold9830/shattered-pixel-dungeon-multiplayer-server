...
public class FloatingText extends RenderedTextBlock {
	...
	public static int getHitReasonIcon(Char attacker, float accRoll, Char defender, float defRoll){
		...
		if (attacker instanceof MirrorImage) wep = ((PrismaticImage) attacker).getHero().belongings.weapon();
		...
		if (defender instanceof PrismaticImage) arm = ((PrismaticImage) defender).getHero().belongings.armor();
		...
		for (Hero hero: Dungeon.heroes) {
			if (hero != null) {
				if (hero.heroClass != HeroClass.CLERIC
					...
					// + 3%/5%
					blessBoost *= 1.01f + 0.02f * hero.pointsInTalent(Talent.BLESS);
				}
			}
		}
